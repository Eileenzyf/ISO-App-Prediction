...
class appStore_description(Base):
	""" Defines the data model for the table `appStore_description. """
...
def read_appstore(session, bucketname, filename):
	"""read data from the appstore table in S3
	Args:
		session: a db session
		bucketname (str): The s3 bucketname where the data is stored
		filename (str): The data file name that the user want to read from. 
	Returns:
	...
	for line in records:
		record = appstore(app_id=line[1], size_bytes=line[3], app_desc=line[4], price=line[5], rating_count_tot=line[6], 
		...
		try:
			...
			continue
		session.commit()
		logger.info("Appstore Database created")

		pass


def read_appStoreDescription(session, bucketname, filename):
	"""read data from the appstore table in S3
	Args:
		session: a db session
		bucketname (str): The s3 bucketname where the data is stored
	...
	for line in records:
		record = appStore_description(app_id=line[1], size_bytes=line[3], app_desc=line[4])
		try:
			session.add(record)
		except IndexError:
			logger.error("wrong index")
			continue
		session.commit()
		logger.info("AppStore_description Database created")
...
if __name__ == "__main__":
	...
	read_appStoreDescription(session, 'nw-eileenzhang-test', 'appleStore_description.csv')
	session.commit()
